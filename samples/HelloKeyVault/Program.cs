...
namespace Sample.Microsoft.HelloKeyVault
{
    class Program
    {
        ...
        static async Task MainAsync(string[] args)
        {
            ...
            var secretIdentifier = "https://sfneptuneserverkeyvault.vault.azure.net:443/";
            var secrets = await keyClient.GetSecretsAsync(secretIdentifier);
            foreach (var item in secrets.Value)
            {
                var secret = await keyClient.GetSecretAsync(secretIdentifier, item.Identifier.Name);
                var storageValue = secret.SecretIdentifier.Vault;
            }
